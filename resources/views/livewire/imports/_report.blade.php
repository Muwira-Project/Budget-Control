@if (! empty($report))
    @if (! empty($report['fatal']))
        <div class="mt-6 rounded-lg bg-red-50 p-4 text-sm text-red-700">
            ...
            <p class="mt-1">{{ $report['fatal'] }}</p>
        </div>
    @else
        <div class="mt-6 rounded-lg bg-green-50 p-4 text-sm text-green-700">
            <p><span class="font-semibold">{{ $report['success'] }}</span> rows imported successfully.</p>
        </div>

        @if (! empty($report['failures']))
            <div class="mt-4 overflow-hidden rounded-lg bg-amber-50">
                <div class="px-4 py-3">
                    <p class="font-semibold text-amber-800">{{ count($report['failures']) }} rows failed:</p>
                </div>
