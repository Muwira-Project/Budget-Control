<div class="py-12">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800 leading-tight">{{ __('Dynamic Master') }}</h2>
                <p class="mt-1 text-sm text-gray-500">Buat jenis master baru secara bebas + penanda AR/AP.</p>
            </div>
        ...
        <x-confirm-modal message="Are you sure you want to delete this master type? All its items will be deleted.">
            <x-bulk-actions :paginator="$this->types" :selected-ids="$this->selectedIds" />
                <div class="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-200">
                @if ($this->types->isEmpty())
                    <p class="p-6 text-sm text-gray-500">No master types yet. Click "Add Master Type".</p>
                @else
...
<td class="w-8 px-6 py-4"><input type="checkbox" wire:click="toggleSelected({{ $type->id }})" @checked(in_array($type->id, $this->selectedIds, true)) class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" /></td>

                                        <td class="px-6 py-4 font-medium text-gray-900">{{ $type->kode }}</td>
                                        <td class="px-6 py-4 text-gray-700">{{ $type->nama }}</td>
                                        <td class="px-6 py-4">
                                            @if ($type->flag_ar)<span class="mr-1 inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">AR</span>@endif
                                            @if ($type->flag_ap)<span class="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">AP</span>@endif
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-right whitespace-nowrap">
                                            <div class="flex items-center justify-end gap-1.5">
                                                <a href="{{ route('master-items.index', $type) }}" wire:navigate title="Items"
                                                ...
                                                <x-action-buttons :edit-href="route('master-types.edit', $type)" :delete-id="$type->id" />
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
