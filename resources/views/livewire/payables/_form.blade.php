<div class="mt-6 grid grid-cols-1 gap-6">
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
            ...
            <p class="mt-1 text-xs text-gray-500">Kosongkan untuk transaksi AP di luar project.</p>
            <x-input-error :messages="$errors->get('project_id')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="akun_id" :value="__('Account Item (COA)')" />
            ...
            @if ($this->projectId)
                <p class="mt-1 text-xs text-gray-500">Menampilkan akun yang telah dialokasikan (approved) untuk project ini.</p>
            @else
                <p class="mt-1 text-xs text-gray-500">Non-project: semua akun ditampilkan.</p>
            @endif
