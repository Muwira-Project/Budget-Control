<div class="py-12">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-800 leading-tight">{{ __('Master Items') }}: {{ $this->masterType->nama }}</h2>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('master-items.create', ['masterType' => $this->masterType->id]) }}" wire:navigate class="inline-flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                    + Add Item
                </a>
            </div>
        </div>
        ...
        <x-confirm-modal message="Are you sure you want to delete this master item?">
            <x-bulk-actions :paginator="$this->items" :selected-ids="$this->selectedIds" />
                ...
                @if ($this->items->isEmpty())
                    <p class="p-6 text-sm text-gray-500">No items yet. Click "Add Item".</p>
                @else
