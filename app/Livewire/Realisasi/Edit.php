...
use App\Http\Requests\Realisasi\UpdateRealisasiRequest;
use App\Models\Akun;
...
{
    public RealisasiModel $realisasi;
    ...
    /**
     * Load the realisasi being edited.
     */
    public function mount(RealisasiModel $realisasi): void
    {
        $this->realisasi = $realisasi;
        $this->projectId = $realisasi->project_id;
        $this->akunId = $realisasi->akun_id;
        $this->vendorId = $realisasi->vendor_id;
        $this->supplierId = $realisasi->supplier_id;
        $this->mandorId = $realisasi->mandor_id;
        $this->investorId = $realisasi->investor_id;
        $this->kategoriId = $realisasi->kategori_id;
        $this->tanggal = $realisasi->tanggal->format('Y-m-d');
        $this->nominal = $realisasi->nominal;
        $this->keterangan = $realisasi->keterangan;
        $this->pihakJenis = match (true) {
            $realisasi->vendor_id !== null => 'vendor',
            $realisasi->supplier_id !== null => 'supplier',
            $realisasi->mandor_id !== null => 'mandor',
            $realisasi->investor_id !== null => 'investor',
            default => 'vendor',
        };
    }

    /**
     * Update the realisasi.
     */
    public function save(RealisasiService $service): void
    {
        $rules = (new UpdateRealisasiRequest)->rules();
        ...
        $service->update($this->realisasi, $validated);

        session()->flash('status', 'Actual updated successfully.');
    ...
    /**
     * Render the realisasi edit page.
     */
    public function render()
    {
        return view('livewire.realisasi.edit');
    }
}
